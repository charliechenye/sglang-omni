...
def _partition_objective(
    ...
    for group in groups:
        ...
        if len(set(typed_keys)) > 1:
            maximum_merged_span = max(maximum_merged_span, max(totals) - min(totals))
        signature.append((typed_keys[0], typed_keys[-1]))
...
def _brute_force_partition(
    ...
    *,
    ...
    """Independent test oracle for every contiguous atomic-bucket partition."""
    ...
    for mask in range(1 << max(0, atomic_count - 1)):
        ranges: list[tuple[int, int]] = []
        start = 0
        for boundary in range(atomic_count - 1):
            if mask & (1 << boundary):
                ranges.append((start, boundary + 1))
                start = boundary + 1
        ranges.append((start, atomic_count))
