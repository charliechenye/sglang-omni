...
@pytest.mark.parametrize(
    ("artifact_field", "error_message"),
    [
        ("ref_spk_embedding", "exactly one speaker embedding"),
        ("ref_code", "exactly one reference code"),
    ],
)
def test_cacheable_voice_prompt_rejects_multiple_artifacts(
    ...
    with pytest.raises(ValueError, match=error_message):
        qwen3_request_builders._cacheable_qwen3_tts_voice_prompt(
            prompt, ref_text="reference text"
        )
